...
def save_images(images: torch.Tensor, path: str, **kwargs):
    ...
    ndarr = (grid.to('cpu').numpy()*255).astype(np.uint8)
...
def save_checkpoint(model, optimizer: torch.optim.Optimizer, save_path: str, epoch: int) -> None:
    """
...
def load_checkpoint(model, optimizer: torch.optim.Optimizer, load_path: str):
    """
...
def load_last_checkpoint(model, optimizer: torch.optim.Optimizer, dir_path: str):
    """
    Loads the last checkpoint from a training process.

    Args:
        model: model in which the weights have to be loaded
    ...
    available_pt_files = [f for f in os.listdir(dir_path) if str.isnumeric(os.path.splitext(f)[0])]
